            ...
            if (d_dump_file.is_open() == false)
                {
                    try
                        {
                            d_dump_filename = "./data/telemetry";
    ...
    if (CRC_ok)
        {
            ...
            if (subframe_ID > 0 and subframe_ID < 6)
                {
                    std::cout <<TEXT_YELLOW<< "New GPS NAV message received in channel " << this->d_channel << ": "
                              << "subframe "
                              << subframe_ID << " from satellite "
                              << Gnss_Satellite(std::string("GPS"), d_nav.i_satellite_PRN) <<TEXT_RESET<< std::endl;
