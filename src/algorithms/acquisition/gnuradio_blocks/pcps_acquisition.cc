    ...
    if (strcmp(d_gnss_synchro->Signal, "1G") == 0)
        {
            ...
            std::cout << "Trying to acquire SV PRN " << d_gnss_synchro->PRN << " with freq " << d_old_freq << " in Glonass Channel " << GLONASS_PRN.at(d_gnss_synchro->PRN) << std::endl;
            return true;
        }
...
//               << ", doppler " << d_gnss_synchro->Acq_doppler_hz
//               << ", magnitude " << d_mag
//               << ", input signal power " << d_input_power<<std::endl;
    d_positive_acq = 1;
