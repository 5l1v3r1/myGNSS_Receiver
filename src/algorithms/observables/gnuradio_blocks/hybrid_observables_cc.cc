                    ...
                    try
                        {
                            ...
                            std::cout << "warning"<<"Observables dump enabled Log file: " << d_dump_filename.c_str();
                        }
                    catch (const std::ifstream::failure &e)
                        {
                            LOG(WARNING) << "Exception opening observables dump file " << e.what();
                            std::cout<<"warning:"<< "Exception opening observables dump file " << e.what();
                            d_dump = false;
                        }
                }
        }
...
hybrid_observables_cc::~hybrid_observables_cc()
{
    ...
    if (d_dump)
        {
            std::cout << "Writing observables .mat files ..."<< std::endl;
            save_matfile();
...
///////////////写obs数据///////////////////////////////////////////////
//    std::cout << "Writing observables .csv files ..."<<std::endl;
//    std::string obsFileName = d_dump_filename;
//    if (obsFileName.size() > 4)
//        {
//            obsFileName.erase(obsFileName.end() - 4, obsFileName.end());
//        }
//    obsFileName.append("_obs.csv");
//
//    std::fstream file;
//    file.open(obsFileName,std::ios::out);
//    if(!file.is_open())
//	   {
...
//	   else
//	   {
//		   std::cout<<"【NOTICE】The file is loaded successfully and the file is being write........"<<std::endl;
...
//                    file<< RX_time[i][j] << ",";
//                    TOW_at_current_symbol_s_aux[k] = TOW_at_current_symbol_s[i][j];
//                    file<< TOW_at_current_symbol_s[i][j]<< ",";
//                    Carrier_Doppler_hz_aux[k] = Carrier_Doppler_hz[i][j];
//                    file<< Carrier_Doppler_hz[i][j] << ",";
//                    Carrier_phase_cycles_aux[k] = Carrier_phase_cycles[i][j];
//                    file<< Carrier_phase_cycles[i][j] << ",";
//                    Pseudorange_m_aux[k] = Pseudorange_m[i][j];
//                    file<< Pseudorange_m[i][j] << ",";
//                    Flag_valid_pseudorange_aux[k] = Flag_valid_pseudorange[i][j];
//                    file<< Flag_valid_pseudorange[i][j] << ",";
//                    k++;
//                    file<< k<< std::endl;
//                }
//        }
//
// //   file << "UTC,"<<"RPN,"<< "TOW" << std::endl;
//	file.close();
////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////
    std::cout << "Writing observables .csv files ..."<<std::endl;
    std::string csvFileName = d_dump_filename;
    if (csvFileName.size() > 4)
        {
            csvFileName.erase(csvFileName.end() - 4, csvFileName.end());
        }
            ...
            for (unsigned int i = 0; i < d_nchannels; i++)
                {
                    ...
                    csvfile<< ","<< Pseudorange_m[i][j] ;
//                    PRN_aux[k] = PRN[i][j];
//                    csvfile<< PRN[i][j] << ",";
//                    RX_time_aux[k] = RX_time[i][j];
//                    csvfile<< RX_time[i][j] << ",";
//                    TOW_at_current_symbol_s_aux[k] = TOW_at_current_symbol_s[i][j];
//                    csvfile<< TOW_at_current_symbol_s[i][j] << ",";
//                    Carrier_Doppler_hz_aux[k] = Carrier_Doppler_hz[i][j];
//                    csvfile<< Carrier_Doppler_hz[i][j] << ",";
//                    Carrier_phase_cycles_aux[k] = Carrier_phase_cycles[i][j];
//                    csvfile<< Carrier_phase_cycles[i][j] << ",";
//                    Pseudorange_m_aux[k] = Pseudorange_m[i][j];
//                    csvfile<< Pseudorange_m[i][j] << ",";
//                    Flag_valid_pseudorange_aux[k] = Flag_valid_pseudorange[i][j];
//                    csvfile<< Flag_valid_pseudorange[i][j] << ",";
//                    k++;
                    if(Flag_valid_pseudorange[i][j] >0)
                    {
                        k = i;
                    }
                ...
                csvfile<< ","<< int64_t(RX_time[k][j])<<","<<TOW_at_current_symbol_s[k][j];
                csvfile<< std::endl;
        }
...
//            std::cout <<"RX_time= "<<it->RX_time<< std::endl;
//
//            std::cout <<"Pseudorange_m= "<<it->Pseudorange_m<<std::endl;
        }
}
...
//                                    std::cout<< "RX_time["<<i<<"]= "<<tmp_double<<std::endl;
                                    tmp_double = out[i][epoch].interp_TOW_ms / 1000.0;
                                    d_dump_file.write(reinterpret_cast<char *>(&tmp_double), sizeof(double));
//                                    std::cout<< "interp_TOW_ms / 1000.0 ["<<i<<"]= "<<tmp_double<<std::endl;
                                    tmp_double = out[i][epoch].Carrier_Doppler_hz;
                                    d_dump_file.write(reinterpret_cast<char *>(&tmp_double), sizeof(double));
//                                    std::cout<< "Carrier_Doppler_hz["<<i<<"]= "<<tmp_double<<std::endl;
                                    tmp_double = out[i][epoch].Carrier_phase_rads / GPS_TWO_PI;
                                    d_dump_file.write(reinterpret_cast<char *>(&tmp_double), sizeof(double));
//                                    std::cout<< "Carrier_phase_rads / GPS_TWO_PI["<<i<<"]= "<<tmp_double<<std::endl;
                                    tmp_double = out[i][epoch].Pseudorange_m;
                                    d_dump_file.write(reinterpret_cast<char *>(&tmp_double), sizeof(double));
//                                    std::cout<< "Pseudorange_m["<<i<<"]= "<<tmp_double<<std::endl;
                                    tmp_double = static_cast<double>(out[i][epoch].PRN);
                                    d_dump_file.write(reinterpret_cast<char *>(&tmp_double), sizeof(double));
//                                    std::cout<< "PRN["<<i<<"]= "<<tmp_double<<std::endl;
                                    tmp_double = static_cast<double>(out[i][epoch].Flag_valid_pseudorange);
                                    d_dump_file.write(reinterpret_cast<char *>(&tmp_double), sizeof(double));
//                                    std::cout<< "Flag_valid_pseudorange["<<i<<"]= "<<tmp_double<<std::endl;
