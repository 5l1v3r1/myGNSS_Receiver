            ...
            if (pmt::any_ref(msg).type() == typeid(std::shared_ptr<Gps_Ephemeris>))
                {
                    ...
                    std::cout<<"(INFO)" << "Ephemeris record has arrived from SAT ID "
                               << gps_eph->i_satellite_PRN << " (Block "
                               << gps_eph->satelliteBlock[gps_eph->i_satellite_PRN] << ")"
                               << "inserted with Toe=" << gps_eph->d_Toe << " and GPS Week="
...
rtklib_pvt_cc::~rtklib_pvt_cc()
{
    ...
    std::string file_name = "./data/eph_GPS_CNAV.xml";
    ...
    file_name = "./data/eph_GPS_L1CA.xml";
    ...
    file_name = "./data/eph_Galileo_E1.xml";
    ...
    file_name = "./data/eph_GLONASS_GNAV.xml";
...
//                             std::cout << TEXT_RED << "(internal) accumulated RX clock offset: " << d_ls_pvt->get_time_offset_s() << "[s]" << TEXT_RESET << std::endl;
                            ...
                            //    }
//                            std::cout<<TEXT_BOLD_RED<<"gnss_observables_map"<<<<TEXT_RESET<<std::endl;
                            bool flag_is_valid_position = d_ls_pvt->get_PVT(gnss_observables_map, false);
                                                    ...
                                                    if (type_of_rx == 1)  // GPS L1 C/A only
                                                        {
                                                            ...
                                                            std::cout<<TEXT_BOLD_RED<<"log_rinex_nav"<<TEXT_RESET<<std::endl;
                                                        }
                                                    if (type_of_rx == 2)  // GPS L2C only
                                                        {
                    ...
                    if (d_dump == true)
                        {
                            try
                                ...
                                {
                                    LOG(WARNING) << "Exception writing  ` dump file " << e.what();
                                }
                        }
                }
        }
